 ...
 * <p>以下两种情况会触发本插件：</p>
 * <ol>
 * <li> 定义在泛型Mapper中的通用方法，并且对应的Entity中有逻辑删除字段。
...
public class LogicallyDeletedInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (hasLogicDeleted(mappedStatement.getId())
            ...
            DeletedDynamicSqlSource deletedDynamicSqlSource = new DeletedDynamicSqlSource(DbType.of(dbType), mappedStatement.getSqlSource(), logicDeleteField, logicDeleteValue, logicNotDeleteValue);
            ReflectUtils.setFieldValue(mappedStatement, "sqlSource", deletedDynamicSqlSource);
        }
    ...
    private boolean hasLogicDeleted(String id) {
        return ReflectUtils.isFieldMapped(StringUtils.substringBeforeLast(id, "."), logicDeleteField);
    }
}
